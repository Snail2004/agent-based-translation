...
- **Canonical source string:** the block's **`clean_text` (fallback `source_text`)** field, NFC-normalized — the EXACT string the model is shown via `render_block_markers` (builder_pilot.py:416). NOT `text`. Offsets index THIS string; if a block lacks both, its anchors are un-mintable → fail-closed.
...
- **Derived keys (all code-computed from SourceAnchor):** `occurrence_ordinal` = rank of same-surface anchors in the block by `char_start`; `position_key = (block_order, char_start, local_ordinal)` where `block_order` = index of the block in the chapter's non-heading sequence and **`local_ordinal`** deterministically breaks ties among turns/events sharing the same `(block, char_start)` — ordered speaker_turns before relation_events, then by the model's array order, canonicalized by code (position_key alone is NOT unique); `mention_id = m_<block>_<occurrence_ordinal>`; `turn_id/event_id/endpoint_id` from `position_key` + role. An in-block frame boundary is a `SourceAnchor` too.
