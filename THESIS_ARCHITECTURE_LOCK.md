...
**Git:** repo ngoài odl-pdf-demo, KHÔNG nested repo mới; nhánh dài hạn `thesis/main`;
commit `P0-01: …` / `docs: …` / `vendor: …` (1 task = 1 commit chính); tag mốc
`P0-done`…`P5-pilot` (viết Chương 3/4 cần trạng thái hệ tại mốc → checkout tag).
Thứ tự khởi đầu: commit 1 = `vendor:` baseline clone nguyên trạng; commit 2 = `docs:`
work protocol (LEDGER/template/vệ sinh) — giữ baseline sạch để diff được với đồ AI-LAB.
