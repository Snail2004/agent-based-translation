...
- **2026-06-20 (zz)** — **EV_D2L_07a RULER-FIX READY** — spec bake-off 3 localizer
  (`find_spans[0]` first-match / `allocate_spans` longest-match / SimAlign semantic) trên GOLD
  NGƯỜI để vá worksheet/override-set EV-06 (user bắt 3 mismark: membership first-match, MNIST
  sub-string, target override-ảo). Verified: D-scorer headline dùng allocate_spans → KHÔNG hư;
  lỗi chỉ ở worksheet EV-06 (find_spans[0]). **4-dim (Claude+GLM hội tụ, Claude giữ tiêu-chí):**
  (1) localizer accuracy 3-way; (2) rep-occ validity = AUTO-CHECK deterministic tách-localizer
  (target = dominant khác toàn-cục nhưng trùng tại occ → re-select/loại-có-lý-do); (3) Metric A
  in-registry so công-bằng ⟂ (4) Metric B out-of-registry (set→bộ) CHỈ SimAlign = bonus RIÊNG,
  CẤM gộp (kẻo SimAlign thắng bằng năng-lực-khác-loại — GLM bắt đúng). **Điểm cộng:** gold
  localization là HUMAN-annotation HỢP LỆ cho user solo (căn-chỉnh ≠ adequacy → khác EV-06,
  vá lỗ no-human-anchor cho riêng phép đo này). Winner = max exact-span-accuracy; tie→longest_match
  (đơn-giản+tất-định); regression trên ca EV-03b-đã-fix → REJECT; SimAlign phải tất-định (EV-05 L5)
  else loại dù accurate → đừng cưới công cụ nặng/unvalidated nếu allocate đủ. Guard narrow: cấm
  `find_spans(...)[0]` cho ATTRIBUTION ngoài allocate internal (presence-check đếm/bool ĐƯỢC —
  GLM rút lại 'cấm tiệt'). 0-API, freeze, EVAL-only; KHÔNG đổi D-scorer headline. Builder-v2
  (prompt/register + do-no-harm signal **DEV-split**+LLM-self-assess + entity-as-generality eval
  Treasure Island) = EV-08 design-doc SONG SONG (parallel, GLM đúng — design-doc không chờ ruler).
  Nối (yy); memory: d2l-scorer-validity-and-remediation-ladder, dont-tune-intervention-on-test-baseline.
- **2026-06-19 (yy)** — **EV_D2L_06 MEMORY_TRADEOFF_01 implementation PASS (machinery);
  FINDING harm% CHƯA có (chờ human-judge mù).** Tái kiểm độc lập (Claude, KHÔNG tin §5):
  frozen DB SHA256 UNCHANGED (`DA0F6878…`), tự chạy 16 tests xanh. **Blind THẬT:**
  worksheet.jsonl/html 0 token xuất xứ; A/B ngẫu nhiên thật (S0/S1=24 vs S1/S0=33, KHÔNG
  luôn A=S0); KEY folder riêng + scorer raise khi thiếu judgment. **Swap THẬT** 57 fwd/57
  rev=114 call judge=gemini-2.5-flash (≠ translator gpt-5.4-mini). **4/114 JSON cắt → repair
  HẸP:** raw kiểm `mt_deferred` = response thật `{label:B_better,conf:0.8,...}` cụt ở reason,
  nhãn CÓ THẬT không bịa, gắn cờ json_error. **Override rule đúng:** 57/57 status_s0==consistent
  AND dom(S1)≠dom(S0), rules→luật keystone có, KHÔNG ép về 59 (rule=source-of-truth; lệch 2-hard
  vs ad-hoc của Claude = ad-hoc lỏng hơn). Dummy `widget coefficient` không-leak (sửa từ `network`
  leak — GLM đúng). **Scorer integration smoke isolated** (folder thật không đụng) → report
  well-formed, tách Gemini-resolved⟂human-agreement, trust 0.80 a-priori (số smoke RÁC vì human
  giả). **FIX reviewer:** override_set.csv (un-blind s0/s1) bị ghi folder MỞ → mv vào KEY/ +
  README phủ cả folder (Gemini không nhiễm; chỉ bảo vệ bước human sắp tới); CodeX sửa build:50.
  Minor: cost=$0 (pricing EV-02 zero, token-gate vẫn đúng). **2 bước USER:** chấm 12 human_suggested
  mù (CHỈ worksheet.html) → score → harm% (agreement≥0.80 tin Gemini, else chấm hết 57; solo→
  diagnostic_only). Nối (xx); memory: memory-injection-precision-cost, dont-tune-intervention-on-test-baseline.
- **2026-06-19 (xx)** — **EV_D2L_06 MEMORY_TRADEOFF_01 READY** — spec đo chi phí chất
  lượng memory trên **59 term override** (S0 `status=='consistent'` nhưng S1 dominant
  đổi form; vd rules quy tắc→luật, framework→khung phần mềm, MNIST-dataset rụng chữ).
  Câu hỏi: improve/lateral/harm % (adequacy vs nguồn EN) → định lượng precision-cost
  của memory (biến trực giác "memory không luôn tốt" thành số; keystone cùng set→bộ
  + rules→luật). **Phương pháp nặng = judge MÙ** (user nêu, Claude+GLM hội tụ): họ
  ngây-thơ (Gemini ≠ gpt-5.4/Claude/GLM — đã dự tranh luận/biết đáp án) + mù xuất xứ
  (blind-at-GENERATION, 3-file tách worksheet/html/KEY-DO-NOT-OPEN, A/B hoán vị seed)
  + đảo-vị-trí 2-call resolve-conservative (mâu thuẫn theo vị trí→ambiguous, KHÔNG
  đếm harm/improve) + reference-aware term-«marked» + cho-phép-equivalent + neo-người
  -mù 12–15 stratified (agreement người↔Gemini quyết tin judge). **GLM bắt đúng +
  Claude NHẬN viết ẩu:** improve/lateral/harm LÀ adequacy → BẮT BUỘC judge; 0-API CHỈ
  cho consistency-drift (form khác), KHÔNG phải harm. **Dummy-example** (network) chống
  spec-public-làm-vỡ-mù (CẤM surface 59 trong template). Post-hoc frozen, EVAL-only,
  **CẤM re-translate/re-run Builder/lọc-injection-từ-S0-test** (dont-tune-on-test: 59
  override CHỈ báo cáo, sửa hệ = arm S1_better a-priori RIÊNG). N=59 + solo → κ=null →
  occurrence-level `diagnostic_only` (nối (ww) L11). Cost-gate Gemini ~118call/$0.12.
  Ngưỡng quyết-định gợi-ý (harm <10%/10–30%/>30%) KHÔNG phải cổng cứng. Follow-up
  `EV_D2L_07_s1_better` (glossary 2-tầng hard/soft/never + do-no-harm canonical,
  a-priori source-side) nếu harm≥10%. Nối (ww)/(vv); memory:
  d2l-scorer-validity-and-remediation-ladder, dont-tune-intervention-on-test-baseline.
- **2026-06-19 (ww)** — **EV_D2L_05 OCC_ALIGN_PILOT_01 implementation PASS (machinery); pilot CHƯA chạy thật.** Tái kiểm độc lập (Claude, KHÔNG tin §5): frozen DB SHA256 UNCHANGED (`DA0F6878…`), tự chạy 12 occ_align tests xanh, production wire SimAlign thật == path test (FakeAligner), artifacts thật (S0 32 / S1 28 rows; gradients→gradient, variable→biến). **Ngữ cảnh GIỮ khi sample-scoped** (`align_independent`/`render_selfreport` đọc nguyên block; `--sample-csv` chỉ lọc occ emit) = rủi-ro-validity #1 loại. **Sample-scoping deviation CHẤP NHẬN + Claude NHẬN spec sai cost:** full self-report = 2041 occ → 292 calls/$2.45-config (không phải ~$0.05); sample-scoped 60-occ đúng mục đích pilot hơn. LOCK enforce + CÓ test: verify≠correspondence (hallucination vs misattribution), S0 mù glossary, model=translator (raise nếu khác), gate a-priori 0.90/0.03 + single_annotator→diagnostic_only, sampler stratified shuffle(seed)+proposer-blind. Sample đa dạng (tier 5 loại, status 4 loại) — KHÔNG lặp bias EV-03b. **Self-report localize = find_spans[0] (first-match) → Claude đo 0/60 at-risk** (sample không surface lặp) → chạy as-is OK; sửa offset/Nth-match chỉ nếu scale. **3 bước USER trước gate:** self-report (cost-gate, gpt-5.4-mini, ~$0.48 thực) → hand-label 60 gold (solo ⇒ diagnostic_only; cần annotator-2 cho headline) → audit. GPU AMD RX 6800: tách task riêng (env torch+cpu, không ROCm/HIP/DirectML trên Win+py3.13); 60 occ CPU đủ, và GPU FP non-determinism nghịch với L5 byte-identical → CPU-deterministic tốt hơn cho tái lập. Nối (vv)/(uu); memory: d2l-scorer-validity-and-remediation-ladder.
- **2026-06-18 (vv)** — **`OCC_ALIGN_PILOT_01` (`TASK_EV_D2L_05_occ_align_pilot.md`) READY: occurrence-level alignment PILOT 1 chương, 2 nhánh NGANG HÀNG (independent SimAlign ⟂ self-report+verify), audit NGƯỜI quyết KHÔNG pre-rank, POST-HOC trên frozen output — KHÔNG re-translate (chốt 3 bên Claude+GLM-5.2+user).** Lý do: surface (kể cả segmentation) MÙ với rendering ngoài-registry (ví dụ sống b003 `set`→S0 "bộ" ∉ registry ⇒ undetected) → chỉ công cụ hiểu-ngôn-ngữ phục hồi được; KHUNG = code liệt kê occurrence NGUỒN EN (`find_spans(language=en)`, bên mạnh), công cụ điền span ĐÍCH. **LOCK chính:** (L4/R1) occurrence-headline ⟺ `min(acc_S0,acc_S1)≥0.90` VÀ `differential=|acc_S1−acc_S0|≤0.03` (ablation đo HIỆU SỐ → bias lệch-nhánh self-report KHÔNG triệt tiêu, accuracy gộp một mình KHÔNG đủ); (L3/R3) **code-verify chỉ chống hallucination, KHÔNG chống misattribution** (`find_spans` presence ≠ correspondence, đúng lớp `tập`⊂`tập dữ liệu` đã tốn 4 task để diệt) → tính đúng self-report chỉ chốt bằng audit người; (L1/R2) gold **proposer-blind + stratified `shuffle(seed=42)` cap ≤3/term** (bài học sampling-bias EV-03b: harness cũ sort alphabet → 6 term, 100% drift); τ a-priori CẤM chỉnh-sau-khi-thấy-kết-quả; (L1 plan-B) **solo → `single_annotator=true` → `diagnostic_only` dù acc≥0.90** (κ cần annotator-2 NGƯỜI trên subsample ~30 occ; 2-session-model = self-consistency, hội đồng bác); nhánh B **model = ĐÚNG model đã dịch** (đổi model = đổi instrument, hết self-report → tách Branch C nếu muốn); nhánh A 0-paid-API+cache, nhánh B qua cost-gate (APP-C01) + preview $ est. **Sequencing: pilot TRƯỚC `EV_D2L_03d_morph_norm`** (alignment dời morphology sang compare-time nhưng KHÔNG xóa nhu cầu định nghĩa tương đương). **Tự đính chính (Claude rút):** "independent aligner sạch nhất" = overclaim (gộp independence vs accuracy — accuracy EN-VI CHƯA đo, đó là việc pilot làm) + rút pre-rank (pilot phải head-to-head). Nối (uu)/(ss); memory: d2l-scorer-validity-and-remediation-ladder, write-tool-unicode-path-phantom.
- **2026-06-17 (uu)** — **EV_D2L_03b PASS + GATE: segmentation ĐỦ cho headline → roadmap đổi (morphological-normalization TRƯỚC alignment) (chốt qua reviewer-audit).** Tái kiểm: determinism khớp, frozen DB hash trước==sau, `metric_version=d2l_translate_score_v2_2`, EN/VI tách + no-silent-fallback (pyvi==0.1.1 pin pipeline+backend), overlay import `surface_match`→hình==số CẤU TRÚC, UI `display_status=diagnostic` (set hết đỏ), 272 tests. **Bug user FIXED:** `learning` hết "học máy" (`{học:14,quá trình học:2}`); `set` "tập" 12→6 (residual `tập dữ liệu` pyvi tách — CodeX ghi thẳng). **GATE (reviewer-audit 60 occ, 30 S0+30 S1): 0/60 surface false-positive THẬT ≤5% ⇒ segmentation ĐỦ cho headline; alignment KHÔNG cần cho headline → ALIGN_PILOT hạ thành trục-2 occurrence-level tùy chọn.** **Phát hiện CHÍNH:** residual hard-tier drift (36 term, D S0 0.750→S1 0.886) KHÔNG phải FP/lỗi-segmentation mà là **biến thể HÌNH THÁI tiếng Việt** (determiner `các X` vs `X`, word-order `vi phân tự động`/`tự động vi phân`, classifier `phép`/`quá trình`) + vài biến thể từ vựng thật → **bước rẻ tiếp theo = `EV_D2L_03d_morph_norm` (0-API, strip determiner + order-insensitive compare), KHÔNG phải alignment.** LOCK-1: reviewer-audit là Claude; tác giả nên hand-label `d2l_seg_audit_sample.csv` cho hồ sơ luận văn (dự kiến ~0 FP). Thứ tự mới: morph-norm → (tùy chọn) ALIGN_PILOT cho occurrence-level + đồng âm. Nối (tt)/(ss); memory: d2l-scorer-validity-and-remediation-ladder.
- **2026-06-17 (tt)** — **EV_D2L_03b siết 6 điểm trước implement (CodeX review, đồng ý hướng — chốt guardrails kỹ thuật, KHÔNG đổi phương pháp).** (1) **Metric version collision (CodeX bắt đúng — Claude verify):** report hiện ĐÃ là `d2l_translate_score_v2_1` (từ EV-D2L-03 term-policy; Claude báo nhầm "v2" lúc review 03) → segmented matcher PHẢI bump **`d2l_translate_score_v2_2`** để phân biệt report cũ/mới. (2) **Tách matcher EN/VI:** `find_spans(text,needle,language="en"|"vi")` — source Anh giữ word-boundary, chỉ TARGET tiếng Việt mới segment (segment EN = rủi ro thừa). (3) **Dependency pin cho CẢ pipeline LẪN app backend** (overlay import matcher) + **CẤM silent fallback regex** khi thiếu segmenter (fallback = đổi thước đo ngầm → fail-loud). (4) **UI tier semantics:** ĐỎ chỉ cho hard-tier drift; soft/ignore (vd `set`) → xám/diagnostic (dùng `constraint_strength` đã có trên span). (5) **Perf budget** (overlay từng timeout A02): segment cache theo (text,version), block ≤~0.3s / chapter ≤~4s, đo trong acceptance. (6) **KHÔNG hứa "hết false tuyệt đối":** segmenter có lỗi riêng (vd có thể tách sai `tập dữ liệu`) → acceptance cam kết GIẢM lớp lỗi đo-qua-audit + ca sai ghi residual-list, không assert zero. Tất cả gói vào `TASK_EV_D2L_03b_segmented_matcher.md` (L1–L6). Nối (ss); memory: d2l-scorer-validity-and-remediation-ladder.
- **2026-06-17 (ss)** — **`EV_D2L_03b` chuyển PARADIGM: surface-matcher v2.1 dựa trên Vietnamese WORD SEGMENTATION (xóa lớp va-chạm-âm-tiết) + BÁC translator-self-emit + alignment = trục-2 audit-gated (chốt 4 bên: Claude + CodeX + 2 external agent, sau tranh luận đầy đủ).** **(1) Gốc rễ (user phát hiện set→"tập" dính "tập trung"; Claude verify):** tiếng Việt cách nhau bằng ÂM TIẾT không phải TỪ → word-boundary regex coi mỗi âm tiết 1 token → "tập"(set) khớp trong "tập trung"/"tập dữ liệu"; **257/1608 term có dạng dịch 1 âm tiết**. Subsumption (03b cũ) chỉ sửa khi cái-chứa LÀ term registry (machine learning), KHÔNG sửa âm-tiết-trong-từ-thường → vá lẻ = vô tận. **(2) Giải pháp toàn diện = Vietnamese word segmentation** (pyvi/underthesea, F1 ~97.5–98.3% verified, 0-API, pin→deterministic): segment đích → match theo TOKEN-TỪ → "tập_trung" 1 token → "tập" hết khớp; phủ CẢ lớp 257 term + tự sửa luôn learning/"học máy". Eval-overlay cleanup HẠ CẤP (chỉ lỗi nghĩa Builder); shared `surface_match.py` scorer+overlay dùng chung (hình==số cấu trúc, sửa luôn việc overlay over-highlight nesting đang vỡ). **(3) BÁC translator-self-emit alignment (cả 2 external agent đề, Claude chặn, cả 2 nhận sai):** S1 emit alignment → prompt S1 ≠ S0 → cải thiện có thể do tự-giám-sát chứ không do memory → **bẩn ablation, mất luận điểm**; self-report ≠ ground-truth; đo S0/S1 bằng 2 method khác nhau phá so sánh; vi phạm observe⊥compute (nn) + "đo ngoài model" (memory). ⇒ phép đo NGOÀI model + CÙNG method cho S0 và S1. **(4) Alignment = trục-2 audit-gated, KHÔNG headline mặc định:** EN-VI alignment accuracy CHƯA thiết lập (search: chỉ có EN-DE…) → KHÔNG giả định ngưỡng; cost nhỏ ($1.3/5 phút — Claude nhận "nặng" là sai, rào cản là ACCURACY không phải cost). **GATE đo được:** audit hard-tier FP-rate ≤5% → segmentation đủ headline; 5–15% → ALIGN_PILOT cho residual; >15% → alignment cần headline. **(5) LOCK kỹ thuật (3 external + 1 Claude):** IAA **neo NGƯỜI** (tác giả hand-label; κ≥0.7 CHỈ khi có annotator-2 NGƯỜI; CẤM 2-session-model gọi là IAA = self-consistency, hội đồng bác); audit **CẢ S0+S1** (30×2, profile FP khác nhau); **dual-axis** (luận văn báo cáo CẢ segmentation-headline LẪN alignment-axis kèm error-bar khi pilot xong — gate chỉ quyết cái nào headline); **residual-list KHÔNG hot-fix lẻ** (`d2l_residual_falsepos.csv`, audit theo đợt — kỷ luật giữ "đổi compiler" không trượt về vá vô tận). **(6) Builder-policy confound (external agent đề, Claude sửa cơ chế):** form-hợp-lệ-không-liệt-kê (vd set→"bộ") lộ ra là UNDETECTED (under-count) KHÔNG phải drift; xử lý bằng audit-gộp (phân loại match/drift/undetected/builder-multi-variant), không cần task nặng riêng. **Thứ tự:** EV_D2L_03b-v2 (segment+audit+gate) → ALIGN_PILOT_01 (external, cùng-method, SimAlign vs LLM-aligner) nếu gate cần. Nối (rr)/(pp)/(qq); memory: d2l-scorer-validity-and-remediation-ladder.
- **2026-06-17 (rr)** — **`EV_D2L_03b` surface-matcher v2.1 = SHARED matcher (scorer+overlay 1 nguồn sự thật) + cross-term subsumption (longest-first across MỌI term) + eval-overlay variant cleanup; 0 API, alignment VẪN là Cấp 3 (chốt 3 bên: user thấy overlay gán "học máy" cho learning; Claude chẩn 2 nguyên nhân + đề longest-source-match; CodeX chốt shared-matcher + "phải cả pollution lẫn containment" + đừng-claim-occurrence-level).** **(1) Lỗi:** `learning.allowed_variants` chứa nhầm "học máy" (pollution, họ calculus⊃phép tính) + "learning" khớp BÊN TRONG "machine/deep learning" (containment: **202/330=61%** occurrence learning thực ra là ML/DL). **(2) Phát hiện cấu trúc (Claude tự kiểm code):** scorer (`d2l_translate_score.py`) và overlay (`thesis_overlay.py`) MỖI nơi giữ bản sao `_MASK_PATTERNS`/`_mask_non_prose`/`_find_matches`; overlay dùng IGNORECASE + **KHÔNG longest-non-overlap** → overlay **over-highlight** form lồng mà scorer EV-D2L-02 đã triệt ⇒ **hình≠số ĐÃ vỡ cho nesting/containment**, không chỉ ca learning. EV-D2L-03 mới chỉ bảo vệ CON SỐ (learning=soft, ngoài hard headline). **(3) Fix (0 API, deterministic, KHÔNG re-translate):** tách `pipeline/eval/surface_match.py` dùng CHUNG (mask + normalize **length-preserving, honor case_sensitive bằng cờ regex chứ KHÔNG casefold → giữ offset cho overlay** + `allocate_spans` joint theo block: gom mọi candidate, sort dài→ngắn, cấp phát non-overlap → mỗi span 1 owner). Source: machine learning giành span chứa learning. Target: "học máy" thuộc machine learning, "học" lẻ thuộc learning. **Phải CẢ pollution-cleanup (eval-overlay bỏ learning⊅học máy) LẪN subsumption** — thiếu 1 trong 2 chưa đủ (xoá variant thì learning vẫn ăn "học" trong "học máy"; chỉ longest-source thì target vẫn ô nhiễm). **(4) Chống tái-phân-kỳ:** test cấu trúc `test_overlay_matches_scorer_forms` — form overlay highlight == keys forms_used scorer cho cùng term+block. **(5) KHÔNG claim occurrence-level:** highlight chỉ nghĩa "term nguồn xuất hiện + form đích xuất hiện trong block (surface, không alignment)"; muốn occurrence-level phải `ALIGN_PILOT_01`. **(6) Thứ tự:** EV_D2L_03b (matcher v2.1) → held-out validation term-policy (0-API) → `EV_D2L_04` pilot 1 chương (API, cost-gated) → full → ALIGN_PILOT_01. Nối (pp)/(qq); memory: d2l-scorer-validity-and-remediation-ladder.
- **2026-06-17 (qq)** — **EV_D2L_02 hygiene LANDED (D_surface_v2) + Cấp 2 `EV_D2L_03` spec = 0-API EVAL-OVERLAY (tôn trọng FREEZE) + headline hard-tier + re-translate STAGED small-pilot→full (chốt review + spec, user quyết staging).** **(1) EV_D2L_02 PASS (47620f4):** longest-match + mask URL/code + honor case_sensitive + terms_all (overlay green reachable); D S1 0.701→0.739 (drift 213→184), AI source-blocks 80→11; mask KHÔNG ăn prose (so clean-matcher: 1 flip trong math). **Review bắt lỗi:** §5 CodeX đổ "B giảm 0.832→0.778" cho masking là SAI/chưa-kiểm — thực do re-score THIẾU `--gold-variants` (gốc: §4 spec của Claude thiếu cờ); với cờ → B **hygiene-neutral** (S1 0.832→0.834, D không đổi). Reviewer regenerate report kèm cờ + vá §4; tái khẳng định luật "headline reviewer phải recompute, không tin §5". **(2) `EV_D2L_03` (Cấp 2, 0 API, re-score output cũ):** `constraint_strength ∈ {hard, soft, preserve, entity, ignore_for_consistency}` gán bằng classifier **a-priori** (do_not_translate / term_type=code_api → preserve; proper_noun → entity (D2L entities table rỗng); abbreviation → soft (giữ acronym, vd AI); term multi-word → hard; single-word → stoplist→ignore / hard_allowlist→hard / else soft) — **KHÔNG nhìn output S0/S1** (tuning-on-test guard). Headline `D_surface_v2` = **chỉ hard-tier** + `by_tier` breakdown minh bạch. **(3) FREEZE-respecting cleanup:** `glossary_entries`/`entities` FROZEN (triggers insert/update/delete — xác minh DB thật) ⇒ dọn pollution (calculus⊅"phép tính", rules canonical→"quy tắc", …) là **EVAL-OVERLAY CSV** (`data/eval/d2l_glossary_fixes.csv` remove_variant/set_canonical + `d2l_term_stoplist.txt`/`d2l_term_hard_allowlist.txt`/`d2l_term_policy_overrides.csv`), scorer áp lúc chấm, **KHÔNG ghi DB** → memory agent-xây bất biến + đo NGOÀI model. Mọi tier/fix phải justify **linguistic/a-priori**, CẤM justify bằng "nó drift trên benchmark". **(4) Re-translate STAGED (user 2026-06-17):** full 4-chương để SAU; **test nhỏ TRƯỚC** = `EV_D2L_04` pilot **1 chương** (cost-gated + preflight prompt review per (ll)/(pp)). EV_D2L_03 thuần 0-API (chỉ scoring/policy); hiệu lực injection (canonical fix, soft=gợi-ý) kiểm ở pilot, KHÔNG ở task này. Nối (pp) thang remediation; memory: `d2l-scorer-validity-and-remediation-ladder`, `dont-tune-intervention-on-test-baseline`.
- **2026-06-17 (pp)** — **Highlight overlay + metric D = MỘT logic block-level surface-match (KHÔNG alignment) → ≥70% "drift" của S1 là artifact đo; HẠ nhãn `D_surface_v2` + thang sửa 3 cấp (hygiene→term-policy→alignment); TÁI DÙNG metric chuẩn (không tự phát minh) + giữ phép đo NGOÀI model (chốt 3 bên: user truy "highlight/thước đo có chính xác không, có nên tự thiết kế metric"; Claude audit code+DB thật; CodeX chặn 2 chỗ Claude quá đà — tuning-on-test + alignment-first).** **(1) GỐC (Claude tự kiểm `pipeline/eval/d2l_translate_score.py:336` `_score_registry_consistency` trên DB thật `data/jobs/d2l_p1`):** `translation_runs` chỉ có `output_text` (KHÔNG alignment); scorer lấy mọi block có chuỗi source rồi đếm form registry xuất hiện BẤT KỲ ĐÂU trong target block đó → bag-of-words trong block, KHÔNG tương ứng source→target. Overlay đọc cùng forms_used → hình==số (overlay TRUNG THÀNH với metric; lỗi nằm ở metric, không phải UI). **(2) ĐO được ≥70% trong 213 drift của S1 là artifact:** 125 **substring-nesting** (canonical ⊂ allowed-variant: `chính xác`⊂`độ chính xác`, `truyền ngược`⊂`lan truyền ngược` → form dài kéo form ngắn → ≥2 form → drift BẢO ĐẢM dù dịch nhất quán); URL/case (`AI`: scorer casefold + coi `.`=ranh-giới → "ai" trong `discuss.d2l.ai` khớp "AI"; thực đo **69/80 block "AI" là khớp NHẦM URL**, 9 thật); 25 **cross-term leakage** (`calculus` drift vì `phép tính` — bản dịch ĐÚNG của `calculations` cùng block — bị Builder liệt nhầm vào allowed_variants). Cột `glossary_entries.case_sensitive` + `evidence_span_ids_json` ĐÃ có nhưng scorer KHÔNG đọc (case_sensitive=0 cả 1608 term; evidence span bỏ qua, rescan cả sách). Overlay chỉ nhận `worst_terms` (drift+undetected) → term `consistent` không tới overlay → hiện xanh-dương "unscored", màu xanh-lá BẤT KHẢ HIỆN. **(3) HẠ CLAIM:** đổi tên D hiện tại → **`D_surface_v2` = diagnostic/pilot block-level, KHÔNG phải số headline bảo vệ**; B (tar-vs-gold) đáng tin hơn D vì gold curated hơn registry + B đo adherence-presence (miễn nhiễm nesting) nhưng VẪN block-level proxy; P3-D2L headline (B 0.764→0.832, D 0.593→0.701) = pilot, tín hiệu SẠCH duy nhất hiện giờ là undetected 27→1. **(4) THANG SỬA 3 CẤP — KHÔNG nhảy thẳng alignment (CodeX đúng; EN-VI khác trật-tự-từ/chèn-bỏ/term-nhiều-token → aligner cũng sai, phải hiệu chuẩn audit tay như judge):** Cấp 1 `EV_D2L_02` (0 API, **chấm lại `translation_runs` cũ, KHÔNG re-translate**): longest-match+non-overlap, mask URL/code/inline/label/math, honor case_sensitive, full per-term report (overlay tô consistent thật), UI "detected target surface in this block", đổi metric_version v2. Cấp 2 `EV_D2L_03`: `constraint_strength` (hard/soft/preserve/entity/ignore_for_consistency) — headline chỉ chấm `hard`; dọn glossary (bỏ `phép tính` khỏi calculus, `rules→quy tắc` hoặc rời strict, `AI→preserve/soft`); ĐÂY mới là chỗ có thể re-translate (cost-gated). Cấp 3 `ALIGN_PILOT_01`: SimAlign/awesome-align trên 50–100 occurrence vs audit tay TRƯỚC khi nâng occurrence-level. **Lưu ý:** Cấp 1 KHÔNG làm calculus xanh — leakage cross-term chỉ hết ở Cấp 2. **(5) GUARD tuning-on-test (CodeX bắt, Claude nhận sai):** Claude đề "chỉ inject term mà S0 bị trôi" — nếu đo S0-drift trên CHÍNH benchmark rồi chọn term cho S1 = tuning-on-test/adaptive-to-baseline dù không đụng gold. Cho phép: S0-analysis chỉ để HẬU KIỂM/giải-thích hoặc học policy trên DEV; benchmark chính khóa policy a priori (termhood/tần-suất/đặc-thù phía source); nếu dùng S0 → arm RIÊNG `S1_adaptive_after_baseline`, không trộn với S1 autonomous-from-source. Memory: `dont-tune-intervention-on-test-baseline`. **(6) TÁI DÙNG, KHÔNG tự phát minh metric + đo NGOÀI model:** đóng góp KT của khóa luận = **adapter mỏng deterministic + evaluation protocol** (định nghĩa đúng mẫu số: scope, không rò gold, không chấm AI trong URL, không pin từ phổ thông, không đồng nhất "nhất quán"≠"dịch đúng") — không repo nào tự biết cho pipeline này. Generic: sacreBLEU (chrF/TER), COMET (chỉ khi có reference người tin được — d2l-vn cộng đồng gần chắc NẰM TRONG training data → contamination → BLEU/COMET vô nghĩa làm headline, chỉ neo sanity yếu). Terminology: giữ scorer deterministic + Alam & Anastasopoulos 2021 (arXiv 2106.11891: exact-match + window-overlap + TERm, GIẢ ĐỊNH annotated reference — KHÔNG cắm thẳng được cho glossary auto-extract → adapter là bắt buộc) + WMT terminology task (term-set CURATED, không ép mọi từ phổ thông về 1 canonical). Alignment hậu kỳ: SimAlign/awesome-align (arXiv 2004.08728, 0 API). **Adequacy = TÁI DÙNG Gemini judge sẵn có (reference-free), KHÔNG dựng COMET infra mới**; "nhất quán SAI = sai đều" → cần trục adequacy tách khỏi consistency. **(7) Mandatory vs suggestion = phân tầng theo loại term VÀ miền:** kỹ thuật pin tên-riêng+thuật-ngữ-đơn-nghĩa, mềm/loại từ phổ thông-đa-nghĩa; văn học chỉ pin entity/tên (ECS), từ vựng tự do. **Áp dụng: `TASK_EV_D2L_02_scorer_hygiene`** (IN: 5 fix hygiene + rename; OUT: term-policy + alignment + adequacy + re-translate). Nối (nn).6 trục-D-nhìn-được, (ii) scope=scope, (ll) cost-as-gate (re-score 0-API trước, re-translate chỉ khi đổi policy); memory: `d2l-scorer-validity-and-remediation-ladder`, `dont-tune-intervention-on-test-baseline`, `scoring-scope-equals-production-scope`, `green-tests-can-hide-dead-integration`, `prompt-memory-design-is-first-class`.
