...
def _render_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# Markdown Normalizer Corpus Check v1",
        "",
        f"Repeat count: {report['repeat']}",
        "",
        "| Source | Deterministic | Units | Translatable | Review | Blocks | Pandoc coverage | Seconds |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in report["rows"]:
        lines.append(
            "| {source} | {deterministic} | {unit_count} | {translatable_count} | "
            "{review_required_count} | {block_count} | {coverage:.6f} | {duration:.3f} |".format(
                source=row["source"],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the Markdown normalizer over explicit sources")
    ...
    for source_value in args.source:
        ...
        for _ in range(args.repeat):
            ...
            runs.append(normalize_markdown(source, doc_id=_doc_id(source)))
    ...
    report = {
        "schema_version": "markdown_normalizer_corpus_check_v1",
        "repeat": args.repeat,
        "rows": rows,
    }
...
def _doc_id(source: Path) -> str:
    return "markdown_" + "".join(character if character.isalnum() else "_" for character in source.stem).strip("_").casefold()
