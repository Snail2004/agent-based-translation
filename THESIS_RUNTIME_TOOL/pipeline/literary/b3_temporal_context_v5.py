"""Parked-identity-aware sequential B3 request construction."""
...
B3_REQUEST_SCHEMA_VERSION_V5 = "literary_b3_temporal_request_v5"
B3_LIVE_PLAN_SCHEMA_VERSION_V5 = "literary_b3_temporal_live_plan_v5"


def render_b3_temporal_sequential_batch_v5(
    *,
    ...
    response_schema = b3_temporal_response_schema_v5()
    ...
    if int(reserve["prompt_token_reserve"]) > profile.prompt_tokens_per_request:
        raise B3TemporalBudgetError("B3 V5 rendered prompt exceeds configured cap")
    body = {
...
def build_b3_temporal_cross_chapter_bundle_v5(
    *,
    temporal_input: Mapping[str, Any],
    profile: B3TemporalProfileV1,
    ...
    batches = _balanced_batches_v5(
        temporal_input=temporal_input,
        components=components,
        profile=profile,
    )
    requests = [
        render_b3_temporal_sequential_batch_v5(
            temporal_input=temporal_input,
            components=batch,
            profile=profile,
            batch_ordinal=index,
    ...
    if set(covered) != set(expected) or len(covered) != len(set(covered)):
        raise B3TemporalContextError("B3 V5 requests do not exact-cover components")
    plan_body = {
        "schema_version": B3_LIVE_PLAN_SCHEMA_VERSION_V5,
...
def _balanced_batches_v5(
    *,
    ...
    raise B3TemporalBudgetError(
        "B3 V5 could not pack components within the sealed request cap"
    )


__all__ = [
    "B3_LIVE_PLAN_SCHEMA_VERSION_V5",
    "B3_REQUEST_SCHEMA_VERSION_V5",
    "build_b3_temporal_cross_chapter_bundle_v5",
    "render_b3_temporal_sequential_batch_v5",
]
