...
ESTIMATE_NEIGHBOR_SUMMARY_PLACEHOLDER = (
    "A prior chapter summary is available to the digest prompt at runtime. "
    "For this zero API estimate, the exact summary text is not yet known because "
    "the preceding digest has not been generated in this invocation. Reserve a "
    "bounded narrative gist of roughly one hundred and fifty words. It may mention "
    "the chapter setting, active narrator, principal events, important character "
    "state changes, unresolved questions, recurring motifs, and facts that could "
    "affect later translation choices. The real run will replace this placeholder "
    "with the validated summary produced by the preceding chapter and will keep "
    "the same neighbor ordering and K bound. This placeholder is sizing data only; "
    "it must never be written to a digest artifact or presented as story evidence. "
    "Its purpose is to reserve realistic prompt space so the estimate does not "
    "pretend that a neighbor summary is only a few words long."
)
...
def _resolve_digest_neighbors(
    *,
    ...
    in_run_sources: dict[str, str] | None = None,
            ...
            provenance.append(
                {
                    ...
                    "source": (in_run_sources or {}).get(neighbor_id, "in_run"),
                }
            )
            continue
        context_value = (
    ...
    *,
    ...
    for chapter_index, chapter in enumerate(selected):
        ...
        chapter_summaries.append(
            {
                "chapter_id": chapter_id,
                "summary": ESTIMATE_NEIGHBOR_SUMMARY_PLACEHOLDER,
            }
        )
        in_run_summaries[chapter_id] = ESTIMATE_NEIGHBOR_SUMMARY_PLACEHOLDER
        in_run_sources[chapter_id] = "in_run_estimate_placeholder"
    upper_tokens = total_prompt_tokens + len(calls) * config.max_output_tokens
    cost_cap = _estimate_cost_cap(
        prompt_tokens=total_prompt_tokens,
        completion_tokens=len(calls) * config.max_output_tokens,
