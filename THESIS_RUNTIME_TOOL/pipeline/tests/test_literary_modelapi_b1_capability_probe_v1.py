...
from pipeline.literary import modelapi_b1_capability_probe_v1 as probe_module
from pipeline.literary.modelapi_b1_capability_probe_v1 import (
...
SECRET = "literary-modelapi-probe-fixture-secret"
...
def _plan(*, probe_run_id: str = "literary_modelapi_b1_probe_test_001"):
    ...
    with patch.object(probe_module, "_git_text", clean_git):
        return build_literary_modelapi_b1_probe_plan_v1(
...
def _provider_body(*, model: str = "gpt-5.4", content: str | None = None) -> bytes:
    content = content or canonical_json(empty_probe_response_v1())
    return canonical_json(
        {
            "id": "literary-modelapi-probe-request-1",
            "model": model,
            "choices": [
...
class _Sender:
    ...
    def send(self, request):
        ...
        return RawTransportResponse(
            status_code=200,
            headers={"x-request-id": "literary-modelapi-probe-request-1"},
            body=_provider_body(model=self.model, content=self.content),
            request_id="literary-modelapi-probe-request-1",
        )
...
def _unknown_evidence(plan) -> dict:
    ...
    return {
        ...
        "probe_id": "literary_modelapi_b1_unqualified_declaration",
...
def _normal_seal(*, source: dict, evidence: dict, plan) -> dict:
    profile = build_literary_pipeline_profile(
        preset=get_literary_shared_role_preset("literary.b1.entity_inventory"),
        api_source=source,
        capability=evidence,
        prompt_ref={
...
def test_profile_and_plan_bind_exact_handoff_contract() -> None:
    profile = load_literary_modelapi_b1_probe_profile_v1()
    plan = _plan()
    assert profile["shared_core_revision"] == SHARED_CORE_REVISION
    assert profile["profile_revision"] == "v3"
    assert (
        profile["capability_intent"]["capability_revision"]
        == "b1_transport_eed09132_validator_1f85e47c_v3"
    )
    assert canonical_sha256(plan.canonical_schema) == B1_CANONICAL_SCHEMA_SHA256
    assert canonical_sha256(plan.response_schema) == B1_SCHEMA_SHA256
    assert B1_SCHEMA_SHA256 == B1_TRANSPORT_SCHEMA_SHA256
    assert (
        canonical_sha256(list(plan.omitted_transport_constraints))
        == B1_OMISSION_SET_SHA256
    )
    ...
    assert plan.source["source_revision"] == "modelapi_profile_v1"
    assert plan.source["base_url"] == "https://modelapi.vn/v1"
    assert plan.source["physical_quota_bucket_id"] == "modelapi-shared-v1"
    assert plan.seal["authority"] == "capability_only"
    assert plan.seal["implementation_binding"] == _implementation_binding()
    ...
    assert plan.response_schema["required"] == plan.canonical_schema["required"]
    assert plan.response_schema["additionalProperties"] is False
    ...
    assert len(plan.request_body["messages"]) == 2
    assert "Wuthering" not in canonical_json(plan.request_body)
...
def test_clean_consumer_binding_requires_clean_exact_head(monkeypatch) -> None:
    ...
    with pytest.raises(LiteraryCapabilityProbeError, match="clean tracked"):
        build_clean_implementation_binding_v1()
...
def test_unknown_rejected_then_exact_evidence_qualifies_and_resolves(tmp_path) -> None:
    plan = _plan()
    ...
    result = execute_literary_modelapi_b1_probe_once_v1(probe=probe, plan=plan)
    ...
    normal = _normal_seal(
        source=dict(plan.source), evidence=result["capability_evidence"], plan=plan
    )
    assert normal["primary"]["capability"]["verdict"] == "qualified"
    assert not any(
        token in path.name.casefold()
        for path in tmp_path.rglob("*")
        for token in ("registry", "checkpoint", "b1_output", "b2_output", "publish")
    )
...
def test_invalid_json_schema_model_and_http_stay_failed(
    ...
    plan = _plan(probe_run_id=f"literary_probe_{expected_code}")
    probe, ledger = _probe(tmp_path, sender)
    result = execute_literary_modelapi_b1_probe_once_v1(probe=probe, plan=plan)
...
def test_explicit_local_semantic_rejection_stays_failed(tmp_path) -> None:
    plan = _plan(probe_run_id="literary_probe_local_semantics")
    sender = _Sender()
    probe, _ = _probe(tmp_path, sender)

    def reject_after_exact_b1_validation(payload):
        validate_literary_modelapi_b1_probe_payload_v1(payload)
        raise ValueError("synthetic local semantic rejection")

    result = probe.execute_once(
        seal=plan.seal,
        request_body=plan.request_body,
        local_validator=reject_after_exact_b1_validation,
...
def test_same_probe_and_capability_revision_cannot_call_again(tmp_path) -> None:
    ...
    sender = _Sender()
    probe, _ = _probe(tmp_path, sender)
    assert execute_literary_modelapi_b1_probe_once_v1(
        probe=probe, plan=plan
    )["status"] == "qualified"
    with pytest.raises(ContractValidationError, match="already reserved"):
        execute_literary_modelapi_b1_probe_once_v1(probe=probe, plan=plan)
    second_plan = _plan(probe_run_id="literary_modelapi_b1_probe_test_002")
    with pytest.raises(ContractValidationError, match="terminal probe evidence"):
        execute_literary_modelapi_b1_probe_once_v1(probe=probe, plan=second_plan)
    assert sender.calls == 1


def test_qualification_is_exact_to_schema_validator_and_implementation(tmp_path) -> None:
    plan = _plan()
    sender = _Sender()
    probe, ledger = _probe(tmp_path, sender)
    evidence = execute_literary_modelapi_b1_probe_once_v1(
        probe=probe, plan=plan
    )["capability_evidence"]
    ...
    with pytest.raises(ContractValidationError, match="schema mismatch"):
        _normal_seal(source=dict(plan.source), evidence=foreign_schema, plan=plan)
    ...
    fresh_probe, _ = _probe(tmp_path / "tampered", _Sender())
...
def test_canonical_local_validator_retains_omitted_constraints() -> None:
    ...
    duplicate["glossary_candidates"] = [
        {
            ...
            "support_block_ids": ["literary_probe_b001", "literary_probe_b001"],
        }
    ]
    with pytest.raises(
        LiteraryStructuredOutputValidationError, match="non-unique|duplicate"
    ):
        validate_literary_modelapi_b1_probe_payload_v1(duplicate)
    ...
    with pytest.raises(
        ...
        validate_literary_modelapi_b1_probe_payload_v1(empty_ids)
    ...
    with pytest.raises(
        ...
        validate_literary_modelapi_b1_probe_payload_v1(empty_surface)


def test_canonical_schema_and_projection_omission_drift_fail_closed() -> None:
    ...
    ), pytest.raises(LiteraryCapabilityProbeError, match="canonical schema"):
        _plan(probe_run_id="literary_probe_canonical_drift")
    ...
    ), pytest.raises(LiteraryCapabilityProbeError, match="omission set"):
        _plan(probe_run_id="literary_probe_omission_drift")


def test_profile_projection_or_dialect_tamper_fails_closed(tmp_path) -> None:
    profile = load_literary_modelapi_b1_probe_profile_v1()
    for field, value in (
        ("schema_dialect", "json_schema_2020_12"),
        ("schema_sha256", B1_CANONICAL_SCHEMA_SHA256),
    ):
        tampered = deepcopy(profile)
        tampered["capability_intent"][field] = value
        path = tmp_path / f"tampered_{field}.json"
        path.write_text(json.dumps(tampered), encoding="utf-8")
        with pytest.raises(LiteraryCapabilityProbeError, match="intent differs"):
            load_literary_modelapi_b1_probe_profile_v1(path)

    tampered = deepcopy(profile)
    tampered["transport_projection"]["omitted_constraint_count"] = 26
    path = tmp_path / "tampered_projection.json"
    path.write_text(json.dumps(tampered), encoding="utf-8")
    with pytest.raises(LiteraryCapabilityProbeError, match="projection differs"):
        load_literary_modelapi_b1_probe_profile_v1(path)


def test_full_canonical_schema_substitution_never_reaches_transport(tmp_path) -> None:
    plan = _plan(probe_run_id="literary_probe_full_schema_substitution")
