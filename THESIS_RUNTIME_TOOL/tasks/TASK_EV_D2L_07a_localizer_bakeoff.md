...
**CAVEAT TRUNG THỰC (bắt buộc vào luận văn):** trên gold đã sửa, first_match vs longest_match khác nhau ở **0/116** dòng — **hoà tuyệt đối**. Khác biệt MNIST sub-string trước kia là artifact của occurrence sai (b126); ở occurrence đúng nó thành documented-limitation chung cho MỌI localizer. ⇒ chọn `longest_match` là **tiebreak L4 theo nguyên tắc** (policy robust, dominant-by-construction, vá failure-mode sub-string/first-match ở EV-03b/06), **KHÔNG phải "đo được longest hơn first" trên bộ này**. Đừng tuyên bố "longest thắng first bằng số". Bake-off này thật sự chứng minh: (1) bắt+vá 2 bug localization worksheet, (2) đo SimAlign CÔNG BẰNG → không đủ (0.19), (3) surface-match đủ tốt (0.93) & robust với lựa chọn policy. Không chế thêm ca để tách first/longest (= fishing).

**Chốt SimAlign (Claude+user, 2026-06-20) — LOẠI:** đánh giá công bằng (đã vá window bug) → exact-span **0.19** (22/116) vs longest_match **0.93**; region-hit **0.88** (102/116: 46 lệch-biên + 34 phình min-max + 14 trượt thật) — ghi lại để KHÔNG strawman, nhưng kết luận: SimAlign **sai nhiều ở biên + nặng (CPU)** → không dùng cho localization neo-registry. Thế mạnh duy nhất: out-of-registry Metric B 1/2 (orthogonal, không thuộc ruler này). UI bằng chứng: `data/eval/localizer_compare.html`. Quyết định cuối: **dùng longest_match, SimAlign loại.**
