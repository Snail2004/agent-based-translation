...
<!-- S36_6_ROUND2_GO -->
### 36.6-ROUND-2 — TASK for CodeX: ap F1-F4 (user chot 2026-07-04) + re-run election

Ap dung 4 fix, KHONG doi gi khac (watchlist signature, gates, log schema, Gemma profile giu nguyen):
- **F1 (routing):** entry co watchlist_reasons chua `audit_polysemy` -> di THANG context vote (b), khong chay back-translation (a) cho viec QUYET DINH. (Van duoc phep chay bt de ghi log tham khao neu re, nhung ket qua (a) KHONG anh huong winner cua entry polysemy.) Entry chi-collision -> (a) truoc nhu cu, tie -> (b).
- **F2 (exact match):** matched_source = exact casefold match voi source term hoac lemma-loose don gian (singular/plural co hoc: +s/+es). CAM containment/substring.
- **F3 (identity gate):** candidate co normalize(text) == normalize(source_term) (hoac dang plural co hoc cua no) KHONG duoc thang bang (a) — identity back-translation la vo nghia; candidate do van duoc tranh cu trong (b).
- **F4 (nguong phieu):** trong (b), challenger chi thang khi: matched_votes(challenger) >= 2 VA matched_votes(challenger) > matched_votes(incumbent). Khong dat -> `unresolved_tie`, giu incumbent. (Nguong dat SAU khi thay data round-1 — da ghi nhan trong STOP-B-R1, giu label nay trong report.)

Chay lai election tren CA HAI notebook (cache Gemma cu tai su dung duoc cho cac call trung — bt calls phan lon trung, context vote population/class/row/feature se la call moi). Out-dir cu, artifact ghi de OK (round-1 da duoc mo ta day du trong STOP-B-R1; neu muon giu, copy sang *_round1/ truoc).

STOP-B-R2 bao cao: (1) population/regularization/gradient; (2) diff notebook full — 0 flip ngoai watchlist; (3) BANG SO SANH flip round-1 vs round-2 (flip nao bien mat, flip nao moi xuat hien, flip nao giu); (4) so call moi + cache hit; (5) danh sach unresolved_tie. KHONG commit. Tieu chi PASS validation giu nguyen §36.6 Buoc 3: population -> "tổng thể", regularization -> "điều chuẩn", 0 out-of-watchlist, moi flip khac liet ke cho Claude review.
<!-- S36_6_STOP_B_R2 -->
### 36.6-STOP-B ROUND-2 — verified: F1-F4 lam dung viec; population = GIOI HAN MO HINH da do duoc, khong phai bug (Claude, 2026-07-04)

**Verify doc lap:** diff notebook: MLP 4 flip / prelim 1 flip, 0 ngoai watchlist (ca 2). Code diff: F1 (backtranslation_bypassed_polysemy), F2/F3, F4 (top>=2 AND >incumbent AND >second) — dung chot. 7 test pass (re-run). Frozen DB nguyen. $0.

**Hieu qua F1-F4 (round-over-round):** 15 flip -> 5 flip; TOAN BO flip rac round-1 bien mat dung co che du doan: feature (F2), row/class (F3), support/pattern/batch + prelim churn (F4). Khong flip moi. 5 flip song sot deu co bang chung: regularization 2-0 (GOLD ✓), training dien cho trong, error 14-0 'sai số', layer 30-0 'lớp' (NGUOC gold 'tầng' — lop §9 convention, cho human review), product rule 2-0 'quy tắc tích' (khop ledger C3.5 cu).

**population — do duoc gioi han that:** chi 4 cau bang chung trong chuong; Gemma vote TRONG NGU CANH thong ke: quần thể 3, nhóm đối tượng 1 (unmatched), tổng thể 0. Ket cuc `unresolved_tie` + GIU incumbent + o lai watchlist. Day KHONG phai routing/threshold bug — la gioi han kien thuc cua voter local ($0) + n=4 evidence. Co che xu ly DUNG THIET KE: khong du bang chung thi khong flip, va day ca ra watchlist cho nguoi (dung khung san pham §36.4).
