...
Output JSON validate bằng schema; sai → re-ask 1 lần. Pilot đo **json_fail_rate**
(go/no-go #1). Span Resolver, persist SQLite, FREEZE là P2-02 — task này chỉ sinh
**artifact JSON tracked**.

**Lưu ý dữ liệu (CodeX phát hiện, đã xác minh):** `treasure_island_ch01` chỉ là trang
ngăn phần ("Part I", 2 block) — truyện thật bắt đầu ở `treasure_island_ch02` (At the
Admiral Benbow, 28 block). → **Pilot chạy `ch02`** (user chốt: 1 chương trước); nếu
ch02 sạch thì chạy thêm `ch03` để khép go/no-go #1 "2 chương" (cache làm lần chạy lại
rẻ). Full-book sau này vẫn quét qua ch01 bình thường — chương 2 block sẽ cho registry
gần rỗng, không cần xử lý đặc biệt.
...
6. `pipeline/scripts/run_prepass.py` — CLI:
   `--source data/sources/treasure_island/document.json --chapters ch02 --out data/prepass/treasure_island_pilot [--config pipeline/configs/llm_prepass.yaml] [--cache data/jobs/prepass_cache.sqlite3]`
   (match chapter theo SUFFIX chapter_id, vd `ch02` khớp `treasure_island_ch02`).
   **API key:** đọc `OPENAI_API_KEY` từ env; nếu env trống → đọc file `API-KEY.txt` ở
   repo root (`research/agent-based-translation/API-KEY.txt`, ĐÃ gitignore) rồi set env
   trước khi tạo client. CẤM in/log key ra console hay report.
7. Tests offline 100% `pipeline/tests/test_world_builder.py` (fake transport — như P0-02).
8. **Chạy thật chương ch02** (key có sẵn — xem mục 6) → artifact tracked git:
   `data/prepass/treasure_island_pilot/{treasure_island_ch02.json, run_report.json}`
   (+ `treasure_island_ch03.json` nếu chạy bước khuyến khích).
...
```
[ch02_b002] At the Admiral Benbow
[ch02_b003] Squire Trelawney, Doctor Livesey, and the rest of these gentlemen...
```
...
# 1. test_prompt_blocks_and_registry: messages chứa block marker [chXX_bYYY...] +
...
# Chạy thật (key đọc từ env hoặc API-KEY.txt repo root; dán console vào §5):
python -m pipeline.scripts.run_prepass --source data/sources/treasure_island/document.json --chapters ch02 --out data/prepass/treasure_island_pilot
# - exit 0; json_fail_rate <= 0.05; 2 file ghi ra (ch02 + report)
# - report đủ usage/cost/model/seed/fingerprint; chạy lại lần 2 → from_cache toàn bộ
# - CodeX spot-check bằng mắt, ghi vào §5: số term/entity/relation; ch02 PHẢI có
#   Jim (narrator) + Billy Bones/"the captain" + quán Admiral Benbow; relations có
#   xưng hô VI; glossary KHÔNG dính từ phổ thông lộ liễu
# - KHUYẾN KHÍCH (nếu ch02 sạch): chạy thêm `--chapters ch02 ch03` để kiểm tra
#   registry-so-far thật + khép go/no-go #1 "2 chương" — ch02 đã cache nên gần như 0 token
