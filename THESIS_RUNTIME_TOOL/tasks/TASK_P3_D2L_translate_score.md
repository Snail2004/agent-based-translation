...
- **Status:** READY (spec hội tụ 3 bên — đã fold document_profile + base prompt D2L + scope-match)
- **Refs:** LOCK **(ii)** (document_profile KHÔNG fork + base prompt D2L = ceiling per-dataset +
  scope chấm = scope dịch), (hh) (injection occ≥2 dataset-aware + `injection_role` derive code),
...
KHÔNG đụng artifact TI / `app/` / registry frozen (chỉ đọc).
...
# PHẢI PASS (offline, fake transport): block filter; injection occ≥2+role+canonical-only;
# purity_check D2L; guard gold-variants không-injectable; scope chấm=scope dịch; B
# occurrence-weighted; D bắt trôi; A; preflight/ceiling kích hoạt

# Preflight TRƯỚC (dry-run, 0 API call) — phải in ước lượng token + chặn nếu vượt:
...
# Dịch thật S0+S1 (profile cấp base prompt D2L + filter heading+prose + injection):
python -m pipeline.scripts.run_translate --db data/jobs/d2l_p1/memory.sqlite3 \
  --profile technical_d2l_v1 \
  --chapters introduction preliminaries linear_networks multilayer_perceptrons --configs S0 S1
# - persist translation_runs S0/S1; in injection stats + block-type counts

# Chấm B/D/A (scope = heading+prose) + curate variants:
python -m pipeline.scripts.score_run --db data/jobs/d2l_p1/memory.sqlite3 --chapters ... \
