...
| P1-02 | Nạp nguồn D2L (EN gốc→blocks) + glossary gold EVAL-ONLY + báo cáo coverage chọn chương | READY | — | — | Nền móng track D2L (gương P1-01); EN `_origin.md`→blocks (bỏ VI MT thô), `glossary.md`→gold **eval-only (CẤM bơm vào model — test guard)**; nạp HẾT corpus ($0 parse) + report coverage/mật-độ-thuật-ngữ để chốt 4 chương Tầng-2; dịch+World Builder+TAR = task sau; ràng buộc user: "dùng gold để chấm, không ném đáp án cho model" |
