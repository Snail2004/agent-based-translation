...
| P2-D2L | World Builder D2L chế độ thuật ngữ kỹ thuật + chẩn đoán C (Builder-vs-gold) + FREEZE | READY | — | — | Dựng registry bơm (`glossary_entries` 0→N) tự học từ text EN (Directional Lock, gold CẤM bơm); prompt chế độ thuật ngữ (termhood/canonical/abbreviation/do-not-translate code/variants/evidence) KHÁC prompt TI; **dev/test: tune trên `deep_learning_computation` → khóa → benchmark intro+prelim+linear+MLP**; **chẩn đoán C trước khi dịch để gác chi tiêu** (recall/agreement/conflict-list/extra, conflict≠lỗi); FREEZE sau build; dịch+A/B/D+judge = P3-D2L |
